...
export default function VideoTab({ settings }) {
  ...
  const toggleSection = (section) => {
    setOpenSections(prev => {
      // Режим когда открыт только один блок
      const newState = {
        resolution: false,
        advanced: false,
        effects: false
      }
      // Если блок уже был открыт - закрываем все
      if (prev[section]) {
        return newState
      }
      // Иначе открываем только этот
  ...
  const ffArgs = useMemo(() => {
    ...
    return args
      ...
      subsMode, stripMeta, fmt, faststart])
    ...
    <div className="content">
      ...
      <div className="two-col">
        ...
        <div className="card">
          ...
          <SelectRow label="Скорость (preset)" value={preset} onChange={setPreset}
            options={PRESETS} />
          <SelectRow label="Аппаратное ускорение" value={hw} onChange={setHw}
            options={HW_ACCEL} />
      ...
      <div className="card">
        ...
        <SliderRow label="Скорость воспроизведения" min={0.25} max={16} step={0.25} value={playbackSpeed} onChange={setPlaybackSpeed} />
